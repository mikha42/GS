...
public class BreedingControl : MonoBehaviour {
    ...
    IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            {
                // Breed them.
                // The creatures only join this list if they both want to breed, so its safe to assume that the two here are breeding together.
    ...
    private void HandleBreeding(Creature creature1, Creature creature2)
    {
        ...
        float availableFunds = Mathf.Max(creature1.creatureHealth, 0.5f * offspringGenes.Length) + Mathf.Max(creature2.creatureHealth, 0.5f * offspringGenes.Length);
        ...
        {
            GameObject baby = Instantiate(creaturePrefab, spawnpos, Quaternion.Euler(0, 0, Random.Range(-180, 180)));
        ...
        creature1.creatureHealth -= 0.7f * offspringGenes.Length;
        creature2.creatureHealth -= 0.7f * offspringGenes.Length;
