...
public class Creature : MonoBehaviour {
    ...
    {
        GameObject[] foods; //Array of foods
        foods = GameObject.FindGameObjectsWithTag("Food"); //Find all gameobjects that have the tag "Food"
        GameObject closestFood = null; //Closest food variable, set to null because if it isn't set and i compair it, it will return an error otherwise
        float closestDistance = Mathf.Infinity; //Start at infinity, because we compair downward to zero, and if we start at any other number, any food with distance higher than that will be ignored.
        Vector3 currentPosition = transform.position; //The creatures current position.
        foreach (GameObject food in foods)
        {
            Vector3 diffrence = food.transform.position - currentPosition;
            float foodDistance = diffrence.sqrMagnitude; // Get the distance to the food
            if (foodDistance < closestDistance) //Compair to current closest food
            {
                closestFood = food;
                closestDistance = foodDistance; // set to closest food if closer
            }
        }
        //if touching eat
        if (gameObject.GetComponent<Collider2D>().IsTouching(closestFood.GetComponent<Collider2D>()))
        {
